/* ============================================================
   PrinterAI — Main JS  v2（全面 Bug 修正版）
   ============================================================ */

/* ── Auth ── */
const AUTH = {
  CREDS: { username: 'nextop', password: '00009999' },
  KEY: 'printerai_auth',
  login(u, p) {
    if (u === this.CREDS.username && p === this.CREDS.password) {
      sessionStorage.setItem(this.KEY, JSON.stringify({ user: u, at: Date.now() }));
      return true;
    }
    return false;
  },
  ...
  getUser()   { const d = sessionStorage.getItem(this.KEY); return d ? JSON.parse(d) : null; },
...
function initLoginForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="bi bi-hourglass spin"></i> 登入中...'; }

    setTimeout(() => {
      if (AUTH.login(u, p)) {
        Toast.show('歡迎回來！', 'success', 1200);
        setTimeout(() => window.location.href = 'dashboard.html', 1200);
      } else {
        if (errEl) errEl.classList.add('show');
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-box-arrow-in-right"></i> 登入'; }
...
function initDashboard() {
  ...
  if (greetingEl) greetingEl.innerHTML = `您好，<strong>${user?.user || '使用者'}</strong>`;
  ...
  toggle.addEventListener('change', () => {
    ...
    document.querySelectorAll('[data-monthly]').forEach(el => {
      ...
      el.textContent = yearly
        ...
        : parseFloat(el.dataset.monthly).toLocaleString();
    });
